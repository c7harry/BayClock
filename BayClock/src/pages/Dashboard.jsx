...
function injectSampleEntries() {
  // Only run on Netlify (hostname ends with .netlify.app)
  if (
    typeof window !== "undefined" &&
    window.location.hostname.endsWith(".netlify.app") &&
    !localStorage.getItem("entries")
  ) {
    const projects = ["Website Redesign", "Mobile App", "Internal Meeting", "Research"];
    const descriptions = [
      "Design homepage",
      "Implement login",
      "Sprint planning",
    ...
    // Generate 14 days of sample entries (including today)
    const entries = [];
    for (let i = 0; i < 14; i++) {
      // Get the date i days ago, always at midnight
      ...
      for (let j = 0; j < entryCount; j++) {
        ...
        entries.push({
          ...
          start: `${startHour}:00`,
          end: `${endHour}:${mins.toString().padStart(2, "0")}`,
        });
      }
    }
...
// Call sample data injection on script load
injectSampleEntries();